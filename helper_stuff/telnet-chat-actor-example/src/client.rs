...
// todo spawns a client with both a mpsc channel and a oneshot channel
//  mpsc send -> into its handle
//  mpsc recv -> into its data
//  oneoff send = keep and use
//  oneoff recv = start_client
...
async fn start_client(my_handle: oneshot::Receiver<ClientHandle>, mut data: ClientData) {
    ...
    //todo sends a msg to server to register a new client
